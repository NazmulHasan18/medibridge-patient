...
import { ArrowLeft, Video, CreditCard, ClipboardCheck, StickyNote, UserRound } from "lucide-react";
...
export default async function AppointmentDetailPage({ params }: Props) {
  ...
  const canJoin =
    appointment.appointmentStatus === "CONFIRMED" && appointment.consultationType === "ONLINE" && meeting;
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5 py-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          },
          {
            ...
            value: `৳${payment?.amount ?? doctor.consultationFee}`,
            sub: payment?.paymentStatus,
          },
        ].map(({ label, value, sub }) => (
      ...
      <div className="grid grid-cols-2 gap-4">
        <DetailCard title="Appointment Details" icon={<ClipboardCheck className="h-3.5 w-3.5" />}>
          <InfoRow label="Patient" value={appointment.patientName} />

          <InfoRow label="Consultation" value={appointment.consultationType} />
        ...
        <DetailCard title="Doctor" icon={<UserRound className="h-3.5 w-3.5" />}>
              ...
              <div className="mt-3 border-t pt-3">
                ...
                {canJoin && (
                  <Link
                    ...
                    target="_blank"
                    className="inline-flex w-full items-center justify-center rounded-md bg-primary px-4 py-2 text-primary-foreground"
                  >
                    Join Meeting
                  </Link>
                )}
